    ...
    <div className="min-h-screen bg-[#f4f7fb] text-slate-950">
      <header className="border-b border-slate-200/80 bg-white/95 backdrop-blur">
        <div className="mx-auto hidden max-w-6xl items-center gap-8 px-6 py-4 md:flex">
          <Link href="/" className="text-[2rem] font-semibold tracking-tight text-slate-950">
            SideQuest
          </Link>
          ...
          {primaryNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex min-h-[4.5rem] flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-center text-[0.7rem] font-medium transition ${
                  isActive
                    ? "text-blue-700"
